...
def process_file(fasta_file, outdir, log_handle, prefix):
    ...
    trimmed_file = os.path.join(outdir, f"{prefix}_{base_name}.fa")
    ...
    for k in range(50, n + 1):
        prefix_seq = seq_str[:k]
        idx = last200.find(prefix_seq)
        if idx != -1:
            max_len = k
    ...
    trimmed_record = SeqRecord(
        Seq(trimmed_seq_str),
        id=f"{prefix}_{header_id}",
        description=f"{prefix}_{header_id}"
    )
...
def main():
    ...
    parser.add_argument("--prefix", default="run1", help="Prefix to prepend to output filenames and FASTA headers")
    ...
    with open(args.log, 'w') as log_handle:
        ...
        for fasta in input_files:
            process_file(fasta, args.outdir, log_handle, args.prefix)
